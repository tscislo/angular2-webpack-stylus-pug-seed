...
describe('WeatherService', () => {
    ...
    it('should get weather data', inject([WeatherService, MockBackend], fakeAsync((WeatherService: WeatherService, MockBackend: MockBackend) => {
        MockBackend.connections.first().subscribe(c => {
        ...
        MockBackend.connections.last().subscribe(c => {
            expect(c.request.url).toBe('http://api.openweathermap.org/data/2.5/weather?q=kielce&APPID=7620b144f4990ae6bb3242d77b7f493e&units=metric');
            let response = new ResponseOptions({
                body: {
                    "coord": {"lon": 21.01, "lat": 52.23},
                    "weather": [{"id": 803, "main": "Clouds", "description": "broken clouds", "icon": "04n"}],
        ...
        WeatherService.get().subscribe((temps: any) => {
            expect(temps[0]).toBe(7.52);
            expect(temps[1]).toBe(17.52);
        })
        tick();
    })));
